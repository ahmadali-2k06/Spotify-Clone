...
async function playPlaylist(folder) {
  const playlist = allData.playLists.find((p) => p.name === folder);
...
async function playNext() {
  ...
  for (let i = 0; i < allUrls.length; i++) {
    if (decodeURIComponent(currentSong.src).includes(allUrls[i])) {
...
function currentSongEnded() {
  const currentSongPath = decodeURIComponent(
    new URL(currentSong.src).pathname.substring(1)
  );
  ...
  .addEventListener("click", () => {
    if (playListActive) {
      ...
      playLists.forEach((playList) => {
        ...
        console.log(encodeURIComponent(playList.getAttribute("data-folder")));
        if (
          currentSong.src.includes(
            encodeURIComponent(playList.getAttribute("data-folder"))
          )
...
// --- MAIN EXECUTION ---
async function main() {
  allData = await getSongs();

  allData.songs.forEach((song) => {
    makeTrendingSongCard(song.name, song.authors, song.url, song.path);
  });

  allData.playLists.forEach((plaList) => {
    let playListcard = makePopulaArtistCard(plaList.name, plaList.cover);
    playListcard.setAttribute("data-folder", plaList.name);
  });
